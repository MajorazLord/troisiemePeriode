    ...
    Public Sub insertAllIO()
        ...
        For Each row As DataRow In myUser.getDTIO.Rows
            ...
            Else 'Alors d'une sortie'
                ...
                If isTTH(row(8)) Then
                    'TTH/TMN
                    Dim reqNumChargeTTHTMN = New SqlCommand("Insert into NUMCHARGE_TTH_TMN (NUM_CHARGE, NUM_OF) values ('" & row(10) & "', '" & row(4) & "')", New SqlConnection(connS3SQL))
                    Try
                        reqNumChargeTTHTMN.Connection.Open()
                    Catch ex As Exception 'Pb de connexion'
                        reqNumChargeTTHTMN.Connection.Close()
                        Throw New Exception
                    End Try

                    Try
                        reqNumChargeTTHTMN.ExecuteNonQuery()
                    Catch ex As Exception 'id container déjà présent
                        Debug.WriteLine(ex.StackTrace)
                    End Try
                End If
